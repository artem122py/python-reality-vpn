...
> ⚠️ **Beta-предупреждение:** флаг `use_vision = true` включает **XTLS-Vision**, который находится **в разработке**. Реализация **неполная** — **не работает с Happ / NekoBox / v2rayNG**. **Не включайте**, если не готовы к отладке. Оставьте `use_vision = false` для стабильной работы.
